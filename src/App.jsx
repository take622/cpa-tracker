...
/**
 * 【同期の最重要フラグ】
 * このIDを固定することで、PC(Gemini)とスマホ(Vercel)が「全く同じ場所」を読み書きします。
 */
const MASTER_APP_ID = "cpa_tracker_master_fixed_v1";
...
const Mascot = () => {
  ...
  return (
    <div className="flex items-start gap-2 bg-indigo-900 p-2.5 rounded-xl border border-indigo-700 cursor-pointer flex-1 min-w-0 shadow-inner" onClick={() => setIndex(i => (i + 1) % MASCOT_MESSAGES.length)}>
      <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center shrink-0">
        <img src="https://api.dicebear.com/7.x/bottts/svg?seed=CPA&backgroundColor=6366f1" alt="Mascot" className="w-6 h-6" />
      </div>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // 1. 週間設定の同期 (MASTER_APP_IDでフォルダを強制固定)
    const goalRef = doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'settings', 'weeklyGoal');
    ...
    // 2. 今日の勉強量の同期
    const todayRef = doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'dailyLogs', todayStr);
    const unsubToday = onSnapshot(todayRef, (snap) => {
      setTodayStudied(snap.exists() ? Number(snap.data().pages || 0) : 0);
    });

    // 3. 教材リストの同期
    const booksCol = collection(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks');
  ...
  const updateProgress = async (id, val) => {
    ...
    batch.update(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks', id), { currentPage: newVal, updatedAt: serverTimestamp() });
    batch.set(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'dailyLogs', currentDate), { pages: increment(dlt), updatedAt: serverTimestamp() }, { merge: true });
    batch.update(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'settings', 'weeklyGoal'), { remainingTarget: increment(-dlt), lastUpdatedDate: currentDate });
    await batch.commit();
  };
  ...
  const moveTextbook = async (index, direction) => {
    ...
    const batch = writeBatch(db);
    batch.update(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks', textbooks[index].id), { sortOrder: targetIndex });
    batch.update(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks', textbooks[targetIndex].id), { sortOrder: index });
    await batch.commit();
  };
  ...
  const handleSave = (e) => {
    ...
    const runSave = async () => {
      try {
        if (tid) await updateDoc(doc(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks', tid), bookData);
        else await addDoc(collection(db, 'artifacts', MASTER_APP_ID, 'users', user.uid, 'textbooks'), bookData);
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans overflow-x-hidden">
      ...
      <main className="max-w-5xl w-full mx-auto p-3 sm:p-4 flex-grow pb-24">
        ...
        <div className="flex items-center justify-between gap-3 mb-6">
          <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <button onClick={() => setDoc(doc(db, 'artifacts', MASTER_APP_ID, 'users', user?.uid, 'settings', 'weeklyGoal'), { remainingTarget: weeklyGoalBase, lastUpdatedDate: getTodayStr() }, { merge: true })} className="p-1.5 text-slate-300 active:rotate-180 transition-all"><RefreshCw className="w-4 h-4" /></button>
          </div>
      ...
      <div className="fixed bottom-4 left-4 right-4 flex items-center justify-between z-10 pointer-events-none">
        <div className="bg-white/90 px-3 py-1.5 rounded-full border border-slate-100 shadow-sm flex items-center gap-2">
          ...
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest text-nowrap">Sync Ready</span>
        </div>
        <div className="bg-white/90 px-3 py-1.5 rounded-full border border-slate-100 shadow-sm flex items-center gap-1.5">
           ...
           <span className="text-[7px] font-bold text-slate-500 font-mono">UID: {user?.uid.slice(-6).toUpperCase()}</span>
        </div>
      </div>
